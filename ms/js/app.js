...
// Same origin as the high school builder: the draft must not collide with its 'tb.draft'. Name and theme are shared on purpose.
const DRAFT_KEY = 'tbms.draft', AUTHOR_KEY = 'tb.author', THEME_KEY = 'tb.theme';

const store = createStore();
const filters = { search: '', grades: new Set(), commitment: '', interestEvent: '', hardNo: false, unassigned: false, sort: 'score' };
...
async function offerDraft() {
  ...
  if (d.baseVersion === store.server.version) {
    ...
    if (v === 'download') download(`ms-teambuilder-draft-v${d.baseVersion}.json`, JSON.stringify(d, null, 2), 'application/json');
...
async function onConflict(res) {
  ...
  const mine = () => download(`ms-teambuilder-my-changes-v${store.server.version}.json`, JSON.stringify({ baseVersion: store.server.version, serverVersion: res.version, summary: summarizeDiff(store.diff()), state: store.state, savedAt: new Date().toISOString() }, null, 2), 'application/json');
  for (;;) {
...
async function rulesGate(actionLabel) {
  ...
  return confirmDialog('Division B rules exceeded', `<p>These teams break the Division B limits:</p><ul>${problems.map(p => `<li>⚠ ${esc(p.message)}</li>`).join('')}</ul><p>${esc(actionLabel)} anyway?</p>`, `${actionLabel} anyway`, 'danger');
}
async function doExport(kind) {
  ...
  if (kind === 'roster-json') download('ms-roster.json', JSON.stringify(lastExport && !store.dirty && lastExport.version === store.server.version ? lastExport.roster : roster, null, 2), 'application/json');
  if (kind === 'roster-csv') download('ms-roster.csv', toCsv(rows), 'text/csv');
  if (kind === 'events-csv') download('ms-events.csv', toCsv(buildEventRows(store.state, store.responses, store.server.events)), 'text/csv');
...
(async () => {
  if (!CONFIG.apiUrl) {   // middle school backend not deployed yet
    document.getElementById('global-notices').innerHTML = '<div class="notice warn"><b>Middle school backend not connected yet.</b> Deploy the middle school Apps Script (see <code>MS_TEAMBUILDER_SETUP.md</code>) and paste its <code>/exec</code> URL into <code>ms/config.js</code>. The high school builder is unaffected — use the switcher in the top-left corner to go back.</div>';
