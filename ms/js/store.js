...
export const DEFAULT_SETTINGS = {
  teamCap: 15, seniorCap: 5, eventWarnAt: 5,    // Division B: 15 per team, at most five 9th graders (seniorCap)
  teamNumbers: { A: null, B: null, C: null },   // unused in the middle school builder (kept so the state shape matches)
  weights: { commitment: 4, accountability: 3, unlikedEvent: 2, experience: 2, medals: 1, grade: 1 }
};
...
export function buildRoster(state, responses) {
  ...
  const roster = { A: [], B: [], C: [] };
  const rows = [['Team', 'Name', 'Email', 'Grade', 'Events', 'Wants to work with', 'Note']];
  const members = Object.keys(state.members).filter(e => state.members[e].team).map(email => {
    ...
    return { email, team: m.team, name: r.name || email, grade: r.grade || '', workWith: r.workWith || '', note: m.note || '', events: eventsHeldBy(state, email).filter(h => h.team === m.team).map(h => h.event).sort() };
  });
  members.sort((a, b) => a.team === b.team ? a.name.localeCompare(b.name) : a.team.localeCompare(b.team));
  members.forEach(m => { rows.push([m.team, m.name, m.email, m.grade, m.events.join(', '), m.workWith, m.note]); roster[m.team].push({ name: m.name, grade: m.grade, events: m.events }); });
