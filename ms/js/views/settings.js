...
import { allConflicts, BLOCK_GROUPS, SELF_SCHEDULE } from '../schedule.js';
...
export function mount(root, ctx) {
      ...
      <div style="display:grid;gap:12px">
        ...
        <div class="card-panel"><h3>Rules & thresholds</h3>
          <div class="weights" style="grid-template-columns:repeat(3,1fr)">
            <label class="field">Team cap (Division B: 15)<input class="input" type="number" min="1" value="${s.teamCap}" data-s="teamCap"></label>
  ...
  function refreshLive() {
    ...
    if (sched) {
      ...
      sched.innerHTML = (bad.length ? `<span class="chip red">⚠ ${bad.length} overlap${bad.length === 1 ? '' : 's'} in the current roster</span><ul class="diff-list">${bad.map(c => `<li><b>${esc((byEmail[c.email] || {}).name || c.email)}</b> (Team ${c.team}): ${esc(c.events[0])} + ${esc(c.events[1])}</li>`).join('')}</ul>` : '<span class="chip green">no schedule overlaps ✓</span>');
    }
  }
