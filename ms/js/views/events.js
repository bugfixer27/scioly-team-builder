...
export function mount(root, ctx) {
  ...
  function render() {
    ...
    const rows = groups.map(g => `<tr class="type-head"><td colspan="4">${TYPE_LABEL[g.type] || g.type} events</td></tr>` + g.events.map(ev => `<tr><td class="evname">${blockOf(ev.name) ? `<span class="color-dot" style="background:${blockOf(ev.name).color}" title="${esc(blockOf(ev.name).label + ' — same time as ' + blockOf(ev.name).events.filter(o => o !== ev.name).join(', '))}"></span>` : ''}${esc(ev.name)}<small>${ev.slots} slots${blockOf(ev.name) ? ' · ' + esc(blockOf(ev.name).label) : ' · self-scheduled'}</small></td>${TEAMS.map(t => `<td>${renderCell(t, ev, byEmail)}</td>`).join('')}</tr>`).join('')).join('');
