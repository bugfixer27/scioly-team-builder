// Events view: matrix of the 23 Division B events × Team A/B/C with slot chips and a per-team slot picker.
...
export function mount(root, ctx) {
  ...
  function render() {
    ...
    const rows = groups.map(g => `<tr class="type-head"><td colspan="4">${TYPE_LABEL[g.type] || g.type} events</td></tr>` + g.events.map(ev => `<tr><td class="evname">${blockOf(ev.name) ? `<span class="color-dot" style="background:${blockOf(ev.name).color}" title="${esc(blockOf(ev.name).label + ' block — same time as ' + blockOf(ev.name).events.filter(o => o !== ev.name).join(', '))}"></span>` : ''}${esc(ev.name)}<small>${ev.slots} slots${blockOf(ev.name) ? ' · ' + esc(blockOf(ev.name).label.toLowerCase()) + ' block' : ' · self-scheduled'}</small></td>${TEAMS.map(t => `<td>${renderCell(t, ev, byEmail)}</td>`).join('')}</tr>`).join('')).join('');
  ...
  function renderCell(team, ev, byEmail) {
    ...
    const chips = list.map(email => {
      ...
      const cls = 'slot filled' + (hard ? ' hardno' : notInt ? ' notint' : '') + (clash.length ? ' clash' : '');
      const mark = (clash.length ? `<span class="clash-flag" title="${esc('Schedule overlap: also holds ' + clash.join(' and ') + ' in the same time block')}">⚠</span>` : '') + (hard ? '<span class="hardno-flag" title="Listed as a hard no">!</span>' : notInt ? '<span class="dot amber" title="Not in their interests"></span>' : '');
